...
import cycleRestrict from './internal/cycleRestrict';
import Deferred from './internal/Deferred';
import launchEmojiWorkers, { MAX_WORKERS } from './launchEmojiWorkers';
import { generateUniqueId } from './utils';

export type EmojiRlottieParams = {
  size: number;
  noLoop?: boolean;
  quality?: number;
...
type Params = EmojiRlottieParams;
...
const requestMutation = (cb: () => void) => {
  queueMicrotask(cb);
};
const requestMeasure = (cb: () => void) => {
  queueMicrotask(cb);
};

/** Для `animate`: один тик на кадр экрана (как fasterdom+RAF у telegram-tt), без цепочки microtask до следующего paint. */
const scheduleAnimationFrame = (cb: () => void) => {
  requestAnimationFrame(() => {
    cb();
  });
};

const workers = launchEmojiWorkers().map(({ connector }) => connector);
const instancesByRenderId = new Map<string, EmojiRlottie>();
...
export function prewarmEmojiRlottieRuntime(): Promise<void> {
  if (!prewarmPromise) {
    prewarmPromise = Promise.all(
      workers.map((connector) =>
        connector.request({
          name: 'rlottie:ensureRuntime',
          args: [],
        }),
      ),
    )
...
export default class EmojiRlottie {
  ...
  static init(...args: ConstructorParameters<typeof EmojiRlottie>) {
    const [, container, renderId, params, viewId = generateUniqueId(), , onLoad, , , onFrame] = args;
    let instance = instancesByRenderId.get(renderId);

    if (!instance) {
      instance = new EmojiRlottie(...args);
  ...
  private addView(
    ...
    if (container instanceof HTMLDivElement) {
      if (!(container.parentNode instanceof HTMLElement)) {
        throw new Error('[EmojiRlottie] Container is not mounted');
      }
      ...
      if (!container.isConnected) {
        throw new Error('[EmojiRlottie] Shared canvas is not mounted');
      }
  ...
  private calcSizeFactor() {
    ...
    const quality =
      this.params.quality ??
      (isLowPriority && (!size || size > LOW_PRIORITY_QUALITY_SIZE_THRESHOLD)
  ...
  private clearCache() {
    ...
    this.imageData = undefined as unknown as ImageData;
    this.frames = [];
  }
    ...
    animate(() => {
      ...
      if (frameIndex !== this.prevFrameIndex) {
        this.views.forEach((containerData) => {
          const { ctx, isLoaded, isPaused, coords: { x, y } = {}, onLoad, onFrame } = containerData;
